...
func ProcessFile(path string) {
	...
	/* OR Записывать изменения во временный файл:
	+ Можно записывать изменения сразу, а не хранить в памяти -> меньше потребление памяти
	+ При возникновении ошибки при записи, исходный файл не пострадает, а частично записанный файл можно удалить
	- ???
	*/
	if isChanged {
...
func main() {
	...
	//Пауза для прочтения логов в контейнере? | сохранять логи на volume? (готово: mount -> /app/logs) | выводить логи в логи контейнера?
